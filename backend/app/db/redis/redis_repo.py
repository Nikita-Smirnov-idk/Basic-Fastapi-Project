...
class RedisRepository:
    ...
    def hash_refresh_token(cls, token: str) -> str:
        """Вычесляет SHA-256 хэш от (token + pepper)"""
    ...
    async def store_refresh_token(
        ...
        """Сохраняет refresh-токен в Redis и возвращает family_id"""
        try:
            family_data = dict()
            if not family_id:
                ...
                family_old_data = await self.redis_client.get(self.FAMILY_PREFIX + family_id)
            ...
            # Добавляем family_id в set пользователя
    ...
    async def get_refresh_data(self, refresh_token: str):
    ...
    async def block_refresh(self, token: str):
        """Добавляет токен в блок-лист"""
        try:
    ...
    async def is_refresh_blocked(self, token: str) -> bool:
        ...
        try:
            token_hash = self.hash_refresh_token(token)
            exists = await self.redis_client.exists(self.REFRESH_BLOCKLIST_PREFIX + token_hash)
            return exists > 0
        except (ConnectionError, TimeoutError, RedisError) as e:
            logger.warning("Redis error: %s", type(e).__name__)
            raise HTTPException(
    ...
    async def block_family(self, family_id: str, user_id: str):
        """Блокирует всю семью токенов"""
        try:
            pipe = self.redis_client.pipeline(transaction=True)
            user_sessions_key = self.USER_SESSIONS_PREFIX + user_id
